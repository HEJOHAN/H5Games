            ...
            for (M > 0 && (t._publicExtData = c.slice(M, c.byteLength)), t._useParent = !!n.getUint8(), t._anis.length = g, h = 0; h < g; h++) {
                ...
                for (v.totalKeyframeDatasLength = 0, l = 0; l < A; l++) {
                    ...
                    if (v.totalKeyframeDatasLength += C, 0 === S.lerpType || 1 === S.lerpType)
                        for (S.interpolationMethod = [], S.interpolationMethod.length = C, o = 0; o < C; o++) S.interpolationMethod[o] = i.AnimationTemplet.interpolation[D.getUint8()];
    ...
    class T {
        ...
        computeWorldPositions(t, e, i, a, r, s, n) {
            ...
            var A, S, T, b, C, F, w, k, P, L = h[0],
                ...
                O = 0,
                V = 0;
            for (x = 0, P = 2; x < D; x++, P += 6) C = 2 * (A = .1875 * (L - 2 * (B = h[P]) + (R = h[P + 2]))) + (T = .09375 * (3 * (B - R) - L + (O = h[P + 4]))), F = 2 * (S = .1875 * (U - 2 * (E = h[P + 1]) + (N = h[P + 3]))) + (b = .09375 * (3 * (E - N) - U + (V = h[P + 5]))), w = .75 * (B - L) + A + .16666667 * T, k = .75 * (E - U) + S + .16666667 * b, l += Math.sqrt(w * w + k * k), w += C, k += F, C += T, F += b, l += Math.sqrt(w * w + k * k), w += C, k += F, l += Math.sqrt(w * w + k * k), w += C + T, k += F + b, l += Math.sqrt(w * w + k * k), I[x] = l, L = O, U = V;
            if (s && (g *= l), n)
                for (x = 0; x < a; x++) f[x] *= l;
            ...
            for (x = 0, o = 0, u = 0, Y = 0; x < a; x++, o += 3)
                ...
                if (u != v) {
                    v = u;
                    var W = 6 * u;
                    for (C = 2 * (A = .03 * ((L = h[W]) - 2 * (B = h[W + 2]) + (R = h[W + 4]))) + (T = .006 * (3 * (B - R) - L + (O = h[W + 6]))), F = 2 * (S = .03 * ((U = h[W + 1]) - 2 * (E = h[W + 3]) + (N = h[W + 5]))) + (b = .006 * (3 * (E - N) - U + (V = h[W + 7]))), w = .3 * (B - L) + A + .16666667 * T, k = .3 * (E - U) + S + .16666667 * b, X = Math.sqrt(w * w + k * k), K[0] = X, W = 1; W < 8; W++) w += C, k += F, C += T, F += b, X += Math.sqrt(w * w + k * k), K[W] = X;
    ...
    class F extends e.Sprite {
        ...
        init(t, i = 0) {
            ...
            if (t.pathArr.length > 0)
                for (null == this._pathDic && (this._pathDic = {}), h = 0, a = t.pathArr.length; h < a; h++) r = t.pathArr[h], s = new T(r, this._boneList), (n = this._boneSlotDic[r.name]) && ((s = new T(r, this._boneList)).target = n), this._pathDic[r.name] = s;
            if (t.tfArr.length > 0)
        ...
        _createGraphics(t = -1) {
            ...
            if (this._rootBone.update(this._yReverseMatrix || e.Matrix.TEMP.identity()), this._ikArr)
                for (m = 0, g = this._ikArr.length; m < g; m++)(T = this._ikArr[m]).name in b && (T.bendDirection = b[T.name]), T.name in C && (T.mix = C[T.name]), T.apply();
            if (this._pathDic)
